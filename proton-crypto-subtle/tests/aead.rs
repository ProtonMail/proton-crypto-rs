use proton_crypto_subtle::{
    aead::{AesGcmCiphertextRef, AesGcmKey},
    SubtleError,
};
...
fn encrypt_decrypt_with_encoded_ciphertext_successful() {
    ...
    let decoded = AesGcmCiphertextRef::decode(&encoded).expect("Expected no error in decoding");
    let decrypted = key
...
fn encrypt_decrypt_custom_encoding() {
    ...
    let ciphertext =
        AesGcmCiphertextRef::new(iv, data).expect("Expected no error in creating ciphertext view");
    let plaintext = key
        .decrypt(ciphertext, Some(CONTEXT))
        .expect("Expected no error in decrypting");
